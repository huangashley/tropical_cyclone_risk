...
"""
# Climatological thermodynamic experiments.
# Each flag triggers a two-pass computation in gen_thermo():
#   Pass 1  (lightweight, no CAPE_PI): compute rh_mid and chi_denom for all
#           time steps and take their full-period means.
#   Pass 2  (full): run compute_thermo with the climatological arrays.
#
# climo_rh_mid : freeze mid-level RH to its time mean.
#   Effect – chi numerator (sps - sp) and the saved rh_mid field both reflect
#   climatological moisture; inter-annual humidity variability is removed.
#
# climo_vmax : freeze vmax (potential intensity) to its time mean AND freeze
#   the chi denominator (spss - sps) to its time mean.
#   Effect – vmax and the thermodynamic potential driving chi are both held at
#   climatology; only synoptic/interannual temperature and humidity variability
#   in the chi numerator remains.
#
# Both flags can be set simultaneously for a fully climatological chi.
climo_rh_mid = False
climo_vmax   = False
